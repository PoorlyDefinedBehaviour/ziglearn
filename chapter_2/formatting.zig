...
test "print" {
    ...
    try expect(std.mem.eql(u8, list.items, "9 + 10 = 19"));
}
...
test "array printing" {
    ...
    try expect(std.mem.eql(u8, string, "{ 1, 4 } + { 2, 5 } = { 3, 9 }"));
}
...
test "custom fmt" {
    ...
    try expect(std.mem.eql(u8, claude_string, "Claude Shannon (1916-2001)"));
