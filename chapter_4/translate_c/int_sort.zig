...
pub export fn int_sort(arg_array: [*c]c_int, arg_count: usize) void {
    ...
    {
        var i: c_int = 0;
        while (@bitCast(c_ulong, @as(c_long, i)) < (count -% @bitCast(c_ulong, @as(c_long, @as(c_int, 1))))) : (i += 1) {
            {
                var j: c_int = 0;
                while (@bitCast(c_ulong, @as(c_long, j)) < ((count -% @bitCast(c_ulong, @as(c_long, i))) -% @bitCast(c_ulong, @as(c_long, @as(c_int, 1))))) : (j += 1) {
                    if ((blk: {
                        const tmp = j;
                        if (tmp >= 0) break :blk array + @intCast(usize, tmp) else break :blk array - ~@bitCast(usize, @intCast(isize, tmp) +% -1);
                    }).* > (blk: {
...
pub const offsetof = @compileError("unable to translate macro: undefined identifier `__builtin_offsetof`"); // /usr/local/zig/lib/include/stddef.h:104:9
